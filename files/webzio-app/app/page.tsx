...
export default function HomePage() {
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background:'#fff', color:'#111', overflowX:'hidden' }}>
      ...
      {/* ── NAV ── */}
      <nav style={{ position:'fixed',top:0,left:0,right:0,zIndex:200,background:navScrolled?'rgba(255,255,255,0.97)':'rgba(255,255,255,0.6)',backdropFilter:'blur(20px)',borderBottom:navScrolled?'1px solid #f0f0f0':'1px solid transparent',padding:'0 6%',height:64,display:'flex',alignItems:'center',justifyContent:'space-between',transition:'all .35s ease',boxShadow:navScrolled?'0 4px 24px rgba(0,0,0,.06)':'none' }}>
        ...
        <div style={{ display:'flex',gap:28,alignItems:'center' }}>
          {navLinks.map(([l,h])=>(
            <a key={l} href={h} className={`nav-a${activeSection===h.slice(1)?' active':''}`} style={{ color:'#555',textDecoration:'none',fontSize:'.86rem',fontWeight:500 }}>{l}</a>
          ))}
        </div>
        <div style={{ display:'flex',gap:10,alignItems:'center' }}>
          <Link href="/login" className="nav-a" style={{ padding:'8px 16px',color:'#555',textDecoration:'none',fontSize:'.86rem',fontWeight:500 }}>Log in</Link>
          <Link href="/signup" className="btn-primary" style={{ padding:'9px 22px',background:'linear-gradient(135deg,#4f46e5,#7c3aed)',color:'#fff',borderRadius:8,textDecoration:'none',fontSize:'.86rem',fontWeight:700,boxShadow:'0 4px 14px rgba(79,70,229,.35)',display:'inline-block' }}>
            Get Started Free
          </Link>
        </div>
      </nav>
